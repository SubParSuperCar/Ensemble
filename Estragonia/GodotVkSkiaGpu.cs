...
internal sealed class GodotVkSkiaGpu : IGodotSkiaGpu
{
	...
	public unsafe GodotVkSkiaGpu()
	{
		...
		var vkPhysicalDevice = new VkPhysicalDevice(
			GetIntPtrDriverResource(RenderingDevice.DriverResource.PhysicalDevice)
		);
		...
		var vkQueueFamilyIndex = (uint)_renderingDevice.GetDriverResource(
			RenderingDevice.DriverResource.QueueFamily,
			default,
			0UL
		);
		...
		var vkGetInstanceProcAddr = (delegate* unmanaged[Stdcall]<VkInstance, byte*, IntPtr>)
			NativeLibrary.GetExport(vkLibrary, "vkGetInstanceProcAddr");
		var vkGetDeviceProcAddr = (delegate* unmanaged[Stdcall]<VkDevice, byte*, IntPtr>)
			NativeLibrary.GetExport(vkLibrary, "vkGetDeviceProcAddr");
		...
		if (GRContext.CreateVulkan(vkContext) is not { } grContext)
			throw new InvalidOperationException(
				"Couldn't create Vulkan context. Note: SkiaSharp doesn't include Vulkan support on macOS."
			);
	...
	public IGodotSkiaSurface CreateSurface(PixelSize size, double renderScaling)
	{
		...
		var vkFormat = (uint)_renderingDevice.GetDriverResource(
			RenderingDevice.DriverResource.TextureDataFormat,
			gdRdTexture,
			0UL
		);
		if (vkFormat == 0U)
	...
	// https://github.com/godotengine/godot/blob/e4e024a/thirdparty/volk/volk.c#L71-L115
	private static bool TryLoadVulkanLibrary(out IntPtr handle)
	{
		...
		if (!OperatingSystem.IsMacOS() && !OperatingSystem.IsIOS())
			return TryLoadByName("libvulkan.so.1", out handle) || TryLoadByName("libvulkan.so", out handle);

		// On macOS, Godot bundles MoltenVK statically in the executable.
		// Try loading from the main program first to avoid conflicts with an external MoltenVK.
		...
		static bool TryLoadFromMainProgram(out IntPtr handle)
		{
			...
			// Verify that the main program exports Vulkan symbols
			return handle != IntPtr.Zero && NativeLibrary.TryGetExport(handle, "vkGetInstanceProcAddr", out _);
		}
