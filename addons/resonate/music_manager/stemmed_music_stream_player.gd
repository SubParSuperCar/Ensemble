...
static func create(p_bank_label: String, p_track_name: String, p_bus: String, p_mode: Node.ProcessMode,
	p_max_volume: float, p_auto_loop: bool) -> StemmedMusicStreamPlayer:
...
func toggle_stem(p_name: String, p_enabled: bool, p_fade_time: float) -> void:
	if not _stem_table.has(p_name):
		# gdlint:ignore = max-line-length
		push_warning(
			"Resonate - Cannot toggle the stem [%s] on music track [%s] from bank [%s] as it does not exist." % [p_name,
				track_name, bank_label])
		return
...
func get_stem_details(p_name: String) -> Variant:
	if not _stem_table.has(p_name):
		# gdlint:ignore = max-line-length
		push_warning(
			"Resonate - Cannot get the details for stem [%s] on music track [%s] from bank [%s] as it does not exist." % [
				p_name, track_name, bank_label])
		return null
