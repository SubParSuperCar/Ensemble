...
static func configure(p_base, p_streams: Array, p_reserved: bool, p_bus: String, p_poly: bool, p_volume: float,
	p_pitch: float, p_mode: Node.ProcessMode) -> void:
	...
	# Ignored due to legacy code I don't have the energy to deal with today.
...
static func trigger(p_base, p_varied: bool, p_pitch: float, p_volume: float) -> void:
	if p_base.streams.size() == 0:
		# gdlint:ignore = max-line-length
		push_warning(
			"Resonate - The player [%s] does not contain any streams, ensure you're using the SoundManager to instance it correctly." % p_base.name)
		return
...
static func release(p_base, p_finish_playing: bool) -> void:
	...
	if p_finish_playing and has_loops:
		# gdlint:ignore = max-line-length
		push_warning(
			"Resonate - The player [%s] has looping streams and therefore will never release itself back to the pool (as playback continues indefinitely). It will be forced to stop." % p_base.name)
		p_base.stop()
