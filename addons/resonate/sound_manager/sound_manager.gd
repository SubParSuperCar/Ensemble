...
func play_varied(p_bank_label: String, p_event_name: String, p_pitch: float=1.0, p_volume: float=0.0,
	p_bus: String="") -> void:
	var instance = _instance_manual(p_bank_label, p_event_name, false, p_bus, false, null)
...
# gdlint:ignore = max-line-length
func play_at_position_varied(p_bank_label: String, p_event_name: String, p_position, p_pitch: float=1.0,
	p_volume: float=0.0, p_bus: String="") -> void:
	var instance = _instance_manual(p_bank_label, p_event_name, false, p_bus, false, p_position)
	instance.trigger_varied(p_pitch, p_volume)
	instance.release(true)
...
# gdlint:ignore = max-line-length
func play_on_node_varied(p_bank_label: String, p_event_name: String, p_node, p_pitch: float=1.0, p_volume: float=0.0,
	p_bus: String="") -> void:
	var instance = _instance_manual(p_bank_label, p_event_name, false, p_bus, false, p_node)
	instance.trigger_varied(p_pitch, p_volume)
	instance.release(true)
...
# gdlint:ignore = max-line-length
func _instance_manual(p_bank_label: String, p_event_name: String, p_reserved: bool=false, p_bus: String="",
	p_poly: bool=false, p_attachment=null) -> Variant:
	if not has_loaded:
		# gdlint:ignore = max-line-length
		push_error(
			"Resonate - The event [%s] on bank [%s] can't be instanced as the SoundManager has not loaded yet. Use the [loaded] signal/event to determine when it is ready." % [
				p_event_name, p_bank_label])
		return _get_null_player(p_attachment)

	if not _event_table.has(p_bank_label):
		push_error(
			"Resonate - Tried to instance the event [%s] from an unknown bank [%s]." % [p_event_name, p_bank_label])
		return _get_null_player(p_attachment)

	if not _event_table[p_bank_label]["events"].has(p_event_name):
		push_error(
			"Resonate - Tried to instance an unknown event [%s] from the bank [%s]." % [p_event_name, p_bank_label])
		return _get_null_player(p_attachment)
	...
	if event.streams.size() == 0:
		# gdlint:ignore = max-line-length
		push_error(
			"Resonate - The event [%s] on bank [%s] has no streams, you'll need to add one at minimum." % [p_event_name,
				p_bank_label])
		return _get_null_player(p_attachment)
	...
	if player == null:
		# gdlint:ignore = max-line-length
		push_warning(
			"Resonate - The event [%s] on bank [%s] can't be instanced; no pooled players available." % [p_event_name,
				p_bank_label])
		return _get_null_player(p_attachment)
...
func _get_player_from_pool(p_pool: Array) -> Variant:
	if p_pool.size() == 0:
		# gdlint:ignore = max-line-length
		push_error(
			"Resonate - Player pool has not been initialised. This can occur when calling a [play/instance*] function from [_ready].")
		return null
	...
	# gdlint:ignore = max-line-length
	push_warning(
		"Resonate - Player pool exhausted, consider increasing the pool size in the project settings (Audio/Manager/Pooling) or releasing unused audio stream players.")
	return null
